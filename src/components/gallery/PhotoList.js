...
const PhotoItem = ({info}) => {
  ...
  return (
    <div className="group relative h-[350px] cursor-pointer">
      <span onClick={()=>toggleFavorite(info.id)} className="right-3 absolute w-[50px] h-[50px] top-5 cursor-pointer transition-all opacity-0 invisible group-hover:opacity-100 group-hover:visible">
        {!info.isFavorite ? (
          <svg
            width="42"
            height="38"
      ...
      <button onClick={()=>addToCart(info)} className="transition-all opacity-0 invisible group-hover:opacity-100 group-hover:visible absolute left-2/4 -translate-x-2/4 py-5 px-6 bottom-10 text-xl bg-white text-black font-semibold rounded-lg w-[250px]">Add to cart</button>
      <img src={info.url} alt="" className="object-cover w-full h-full" />
